...
// SignalAccepted проверяет, что мир принял НЕ МЕНЕЕ min сигналов аппарата ЗА
// ОКНО НАБЛЮДЕНИЯ check — не «хотя бы один», а «сигналов было столько,
// сколько обязано быть при рабочей каденции».
//
// Один сигнал за окно — тоже дефект, а не успех: аппарат при нормальной
// каденции ~15 с шлёт за 45-секундное окно около трёх сигналов (см.
// expectedSignals в cmd.Check, откуда вызывающий код и берёт min), и
// зависший сразу после первого heartbeat аппарат ОБЯЗАН провалить эту
// проверку — то же самое увидит централизованная сборка при той же
// каденции. Недобор ниже min — обычный провал guaranteed (аппарат не
// прислал сигналы, которые был обязан прислать), а не Skip по нехватке
// наблюдений: та развилка — забота SequenceMonotonic и Cadence, которым
// действительно не из чего судить при <2 точках, а не этой проверки, для
// которой сам факт недобора уже и есть ответ.
//
// v.Condition для порога не годится: аппарат, приславший один heartbeat и
...
func SignalAccepted(v worldapi.View, seq []int64, min int) Result {
	const name = "сигналы приняты миром"
	passed := len(seq) >= min
	detail := fmt.Sprintf(
		"сигналов за окно наблюдения: %d, ожидалось не менее %d, состояние витрины: %s, последний сигнал №%d",
		len(seq), min, v.Condition, v.LastSequence,
	)
